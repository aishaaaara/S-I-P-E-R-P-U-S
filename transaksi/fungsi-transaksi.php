...
<?php
function ambilPeminjaman($kon){
	$sql = "SELECT * FROM pemijaman p INNER JOIN anggota a ON p.id_anggota=a.id_anggota INNER JOIN detail_pinjam dp USING(id_pinjam) INNER JOIN buku b ON dp.id_ buku=b.id_buku where id_pinjam='$id_pinjam'";
	$res = mysqli_query($kon, $sql);
	$data = array();
